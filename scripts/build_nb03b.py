"""Gera o notebook 03B (pré-processamento — artrite COM comorbidades) do zero.

Variante do build_nb03.py para o desenho alternativo do estudo:
  * Caso = idosos com Q079=Sim (com qualquer combinação de outras doenças crônicas)
  * Controle = idosos saudáveis (sem nenhuma das 14 doenças)

Diferenças em relação ao 03 (artrite pura):
  * Usa idosos_artrite.db (4 025) em vez de idosos_artrite_puro.db (494)
  * Razão de balanceamento ~1.08:1 (quase 1:1)
  * Q092 e outras Q* NÃO são constantes → mantidas como features
  * Etapa nova: análise descritiva do número de comorbidades por indivíduo
  * Aviso metodológico explícito sobre data leakage circular

Uso (rodar da raiz do projeto):
    python scripts/build_nb03b.py

Gera/sobrescreve notebooks/03b_preprocessamento_comorbidades.ipynb.
"""
...
NB_PATH = ROOT / "notebooks" / "03b_preprocessamento_comorbidades.ipynb"
...
# ╰──────────────────────────────────────────────────────────────────────────╯
cells.append(md(r"""
# Notebook 03B — Pré-Processamento (Artrite **com** comorbidades)

**Projeto:** Mineração de Dados em Saúde · PNS 2019
**Estudo:** Artrite e Reumatismo em Idosos Brasileiros — *desenho alternativo*
...
O Notebook 03 original usa o desenho **"artrite pura"** — só inclui idosos com artrite
**isolada** (sem nenhuma das outras 13 doenças crônicas). Isso resulta em apenas
494 casos vs 4 332 controles → **razão 8,77:1** (desbalanceamento severo).

Este notebook é a **variante "artrite com comorbidades"**:

| Característica | 03 (puro) | **03B (este)** |
|---|---|---|
| Banco | `idosos_artrite_puro.db` | **`idosos_artrite.db`** |
| Caso | Q079=Sim **E** outras 13 doenças = Não | **Q079=Sim** (qualquer combinação) |
| N (casos) | 494 | **~4 025** |
| Razão de balanceamento | 8,77:1 | **~1,08:1 (quase 1:1)** |
| Q092 (depressão) | Constante → removida | **Varia → mantida** |
| Realismo clínico | Idoso "puro" (raro) | **Idoso real** (com comorbidades) |

**Trade-off:** Ganha-se balanceamento e realismo, mas **introduz-se risco de data leakage circular**
porque as outras variáveis Q* foram usadas como filtro para definir o grupo Saudável.
Discutido em detalhe na Etapa 0 (próxima célula).

## Fluxo geral

| # | Etapa | O que faz |
|---|-------|-----------|
| 0 | **⚠️ Aviso metodológico** | Discussão sobre data leakage circular |
...
| 3.5 | **🆕 Análise de comorbidades** | Quantas comorbidades têm os casos? |
...
| 11 | Encoding | Label Encoding (ordinais) + OHE (nominais) — **Q092 mantida** |
| 12 | Dataset final | Monta X, y e diagnóstico de balanceamento |
| 13 | Exportação | CSV + JSON de rastreabilidade |

> Convenção de cor: <span style="color:#C0392B">**vermelho = Com Artrite (c/ comorb.)**</span>, <span style="color:#27AE60">**verde = Saudável**</span>.
"""))

# ╭──────────────────────────────────────────────────────────────────────────╮
# │ Etapa 0 — Aviso metodológico                                              │
# ╰──────────────────────────────────────────────────────────────────────────╯
cells.append(md(r"""
## 0 · ⚠️ Aviso metodológico — data leakage circular

### O problema

A definição do grupo **Saudável** exige que **todas as 14 variáveis Q\* sejam "Não"**.
Isso significa que dentro deste subgrupo, hipertensão (Q00201), diabetes (Q03001),
colesterol (Q060), etc. são **sempre constantes = Não**.

No grupo **Com Artrite (com comorbidades)**, essas mesmas variáveis **variam livremente**:
um indivíduo pode ter artrite + hipertensão + diabetes, ou só artrite, ou qualquer combinação.

### Por que isso é problema?

Um classificador treinado nessa amostra aprenderia uma regra trivial:

> *Se Q00201 = "Sim" (tem hipertensão), então é "Com Artrite".*

Isso parece um padrão forte, mas é **artefato do desenho**, não da biologia. Na população
geral, alguém pode ter só hipertensão (sem artrite) — mas esse perfil **não existe na nossa
amostra** porque foi excluído da definição de "Saudável".

### Como mitigar?

Três caminhos possíveis no Notebook 04:

1. **Modelo A — completo:** treinar com todas as Q\* como features (este notebook prepara o dataset). Reportar acurácia alta com a ressalva metodológica.
2. **Modelo B — sem Q\*:** excluir todas as Q\* (exceto a target Q079) e treinar só com sociodemográficos, antropometria e hábitos. Menos acurácia, mais válido para população geral.
3. **Análise de sensibilidade:** comparar A vs B e reportar quanto da acurácia vem do "label leakage".

**Decisão neste notebook:** prepararemos o dataset **completo** (Modelo A). A escolha A vs B fica para o NB04.

### Referência

> *Saeb, S. et al. The need to approximate the use-case in clinical machine learning. GigaScience 6(5), 2017.* — Discute como vazamentos de delineamento inflam métricas de modelos clínicos.
"""))
...
DIR_RESULTADOS = '../data/results/preprocessing_comorbidades/'        # saídas deste notebook (pasta separada do 03)
...
ARQUIVOS = {
    'idosos_artrite.db'  : 'Com Artrite',   # target = 1 (Q079=Sim, com qualquer combinação de outras doenças)
...
# ╭──────────────────────────────────────────────────────────────────────────╮
# │ Etapa 3.5 — 🆕 Análise de comorbidades (exclusiva deste notebook)         │
# ╰──────────────────────────────────────────────────────────────────────────╯
cells.append(md(r"""
## 3.5 · 🆕 Análise descritiva das comorbidades

Antes de prosseguir com o pipeline, vamos **caracterizar o grupo Com Artrite**: quantas
das outras 13 doenças crônicas cada indivíduo apresenta?

Isso responde:

1. Qual a fração de "artrite pura" dentro deste grupo maior? (Esses ~494 seriam os do nb03.)
2. Qual é a distribuição de multimorbidade (1, 2, 3, 4+ doenças além da artrite)?
3. Quais comorbidades são as mais prevalentes?

Esses números entram na **Tabela 1 do artigo** (descrição da amostra) e servem para
discutir a generalização clínica dos resultados.
"""))

cells.append(code(r"""
# 13 variáveis Q* além de Q079 (target); aceita 'Sim' / '1' / numérico 1
COMORBS = ['Q00201','Q03001','Q060','Q06306','Q068','Q074','Q088',
           'Q092','Q11006','Q11604','Q120','Q124','Q128']

def _eh_sim(serie):                                          # detecta resposta "Sim" em qualquer formato
    s = serie.astype(str).str.strip().str.lower()            # normaliza string
    num = pd.to_numeric(serie, errors='coerce')              # tenta numérico
    return (num == 1) | s.isin(['1','sim','s'])              # union de critérios

# Filtra só grupo "Com Artrite" (no Saudável todas Q*=Não por definição)
df_art = df_raw[df_raw['Classe'] == 'Com Artrite'].copy()

# Conta comorbidades por linha (soma dos "Sim" entre as 13)
indicadores = pd.DataFrame({c: _eh_sim(df_art[c]) for c in COMORBS if c in df_art.columns})
df_art['n_comorbidades'] = indicadores.sum(axis=1)           # quantas além de Q079

# Distribuição da contagem
dist = df_art['n_comorbidades'].value_counts().sort_index()  # 0, 1, 2, 3, ...
print('='*60)
print('  DISTRIBUIÇÃO DO Nº DE COMORBIDADES (além de artrite)')
print('='*60)
total = len(df_art)
for n, c in dist.items():
    pct = 100 * c / total
    rotulo = 'artrite pura' if n == 0 else f'{int(n)} comorbidade{"s" if n>1 else ""}'
    print(f'  {int(n):2d} ({rotulo:18s}): {c:5,} ({pct:5.1f}%)')
print(f'  {"TOTAL":>21s}: {total:5,}')
print()
print(f'  Média de comorbidades por idoso com artrite: {df_art["n_comorbidades"].mean():.2f}')
print(f'  Mediana                                    : {df_art["n_comorbidades"].median():.1f}')
print(f'  Máximo                                     : {int(df_art["n_comorbidades"].max())}')

# Tabela das comorbidades mais prevalentes (no grupo Com Artrite)
NOMES_DOENCAS = {
    'Q00201':'Hipertensão',    'Q03001':'Diabetes',       'Q060':'Colesterol alto',
    'Q06306':'Doença cardíaca','Q068':'AVC',             'Q074':'Asma',
    'Q088':'DORT',             'Q092':'Depressão',        'Q11006':'Outra doença mental',
    'Q11604':'DPOC',           'Q120':'Câncer',          'Q124':'Renal crônica',
    'Q128':'Outra crônica',
}
prevalencia = []
for col in COMORBS:
    if col not in df_art.columns: continue
    n_sim = int(_eh_sim(df_art[col]).sum())
    prevalencia.append({
        'Código'  : col,
        'Doença'  : NOMES_DOENCAS.get(col, col),
        'n'       : n_sim,
        '%'       : round(100 * n_sim / total, 1),
    })
tab_comorb = pd.DataFrame(prevalencia).sort_values('%', ascending=False)
print('\n' + '='*60)
print('  PREVALÊNCIA DE COMORBIDADES NO GRUPO Com Artrite')
print('='*60)
print(tab_comorb.to_string(index=False))
salvar_tab(tab_comorb, 'etapa3_5_comorbidades_prevalencia.csv')

# ── Visualização: histograma + barras de prevalência ────────────────
fig, axes = plt.subplots(1, 2, figsize=(15, 5))

# Painel A: histograma do nº de comorbidades
sns.histplot(df_art['n_comorbidades'].astype(int),
             bins=range(0, int(df_art['n_comorbidades'].max())+2),
             discrete=True, ax=axes[0], color=COR_ARTRITE, edgecolor='white')
axes[0].set_title('Nº de comorbidades por idoso com artrite', fontweight='bold')
axes[0].set_xlabel('Nº de doenças crônicas além da artrite')
axes[0].set_ylabel('Quantidade de idosos')
for n, c in dist.items():
    axes[0].text(int(n), c + 30, f'{c:,}', ha='center', fontsize=9)

# Painel B: prevalência de cada comorbidade
sns.barplot(data=tab_comorb, x='%', y='Doença', ax=axes[1],
            color=COR_ARTRITE, edgecolor='white')
axes[1].set_title('Prevalência de cada comorbidade', fontweight='bold')
axes[1].set_xlabel('% no grupo Com Artrite')
axes[1].set_ylabel('')
for i, row in tab_comorb.reset_index(drop=True).iterrows():
    axes[1].text(row['%'] + 0.5, i, f"{row['%']:.1f}% (n={row['n']:,})",
                 va='center', fontsize=9)

fig.suptitle(f'Etapa 3.5 — Caracterização da multimorbidade (n={total:,})',
             fontsize=13, fontweight='bold', y=1.02)
plt.tight_layout()
salvar_fig('etapa3_5_comorbidades.png')
plt.show()

# Limpeza: remove a coluna auxiliar para não vazar nos próximos passos
df_art.drop(columns=['n_comorbidades'], inplace=True)
"""))
...
### Por que Q092 (depressão) é **mantida** aqui

No notebook 03 (artrite pura), Q092 é constante = 'Não' pois o critério de inclusão
exclui depressão. Aqui no 03B, o grupo "Com Artrite" inclui qualquer combinação de
comorbidades — então Q092 varia e é uma **feature útil**.

Idem para Q00201 (hipertensão), Q03001 (diabetes), Q060 (colesterol), Q06306 (cardíaca),
Q068 (AVC), Q074 (asma), Q088 (DORT), Q120 (câncer), Q124 (renal), Q11006 (outra mental),
Q11604 (DPOC), Q128 (outra crônica).

⚠️ Lembrar do **aviso da Etapa 0**: essas variáveis são preditores extremamente fortes,
mas em parte por **construção do desenho**, não pela biologia. O modelo terá acurácia
inflada nesta amostra.
"""))
...
# ── One-Hot Encoding (TODAS as variáveis nominais, incluindo Q092) ───
# Diferença vs nb03: Q092 NÃO é mais removida pois varia neste desenho.
cols_ohe = [c for c in VARS_NOMINAIS_OK if c in df_step6.columns]
print(f'\n  OHE ({len(cols_ohe)} variáveis nominais — Q092 incluída):')
...
if cols_ohe:
    ...
    if vars_nao_geradas:
        ...
        print(f'     Causa: constantes nesta amostra.')
...
relatorio = {
    ...
    'pesquisador': 'Pedro Dias Soares',
    'orientador' : 'Prof. Dr. Luis Enrique Zárate — PUC Minas',
    'notebook'   : '03b_preprocessamento_comorbidades',
    'versao'     : 'v1 — variante com comorbidades (idosos_artrite.db)',
    'desenho'    : 'Com Artrite (Q079=Sim, qualquer combinação de outras doenças) vs Saudáveis',
    'aviso'      : 'Risco de data leakage circular nas variáveis Q* — ver Etapa 0 do notebook',
    'parametros' : {
    ...
    'rastreabilidade': {
        ...
        'etapa_8_encoding': {
            ...
            'excluidas_ohe' : [],
            'removidas_df'  : [],
            'observacao'    : 'Q092 e demais Q* mantidas — varia neste desenho (artrite com comorbidades)',
        },
...
cells.append(code(r"""
print('=' * 80)
print('  ✅  NOTEBOOK 03B — PRÉ-PROCESSAMENTO (Artrite c/ comorbidades) CONCLUÍDO')
print('=' * 80)

print('\n  RESUMO DAS ETAPAS:')
print(f'  [1]   Carregamento        : {df_raw.shape[0]:,} registros × {df_raw.shape[1]} colunas')
print(f'  [3.5] Análise comorbid.   : tabela + figura salvas')
...
print(f'  [11]  Encoding            : Label Enc. + OHE (Q092 mantida)')
...
if razao <= 1.5:
    print(f'    ✅ Quase balanceado — RUS opcional no NB04.')

print('\n  COMPARAÇÃO DE DESENHOS (03 puro vs 03B comorbidades):')
print('  ─────────────────────────────────────────────────────────────────────')
print('             | n casos | n controles | razão  | Q* como features')
print('  ─────────  | ------- | ----------- | ------ | ----------------')
print('  03 puro    |    494  |       4 332 | 8.77:1 | NÃO (constantes)')
print(f'  03B (este) |  {(y==1).sum():5,}  |     {(y==0).sum():6,}  | {razao:5.2f}:1 | SIM (variam)')
print('  ─────────────────────────────────────────────────────────────────────')

print('\n  ⚠️  LEMBRETE METODOLÓGICO (Etapa 0):')
print('     Q* (hipertensão, diabetes, etc.) foram filtro para definir Saudável.')
print('     Modelos terão acurácia inflada — discutir como limitação no artigo.')

print('\n' + '=' * 80)
print('  Próximo → Notebook 04: comparar ML em 03 (puro) vs 03B (comorbidades)')
print('  Sugestão: treinar Modelo A (com Q*) e Modelo B (sem Q*) para sensibilidade')
print('=' * 80)
"""))
